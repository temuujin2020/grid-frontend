// app.js — Live + Upcoming from your Render proxy (single, clean copy)
(function () {
  ...
  // Config (URL params override HTML data-* attributes)
  ...
  // Normalize one series object from the proxy into render-ready fields
  function normOne(it, liveFlag) {
    const teamsArr = Array.isArray(it.teams) ? it.teams : (it.teams || []);
    const names = teamsArr.map(t =>
      t?.name || t?.baseInfo?.name || ""
    ).filter(Boolean);

    // Try to read scores (if your proxy includes them on live)
    ...
    if (typeof it.format === "string") {
      ...
      format = it.format.id; // GRID often exposes numeric string ids (1,2,3,5 -> BO1/2/3/5)
    }
    ...
    return {
      ...
      event: eventName,
      format: format ? `BO${format}` : "",
      teams: [names[0] || "TBD", names[1] || "TBD"],
      live: !!liveFlag,
      scoreA: (sA !== null && sA !== undefined) ? Number(sA) : null,
      scoreB: (sB !== null && sB !== undefined) ? Number(sB) : null
    };
  }

  function normalize(list, liveFlag) {
    return (Array.isArray(list) ? list : []).map(it => normOne(it, liveFlag)).filter(x => x.id);
  }
  ...
  async function load() {
    ...
    try {
      ...
      if (pin) {
        const hasPin = (m) => (m.teams.join(" ").toLowerCase().includes(pin));
        live.sort((a, b) => {
      ...
      if (!ctrl.signal.aborted) {
        ...
        // Keep current DOM; you could also show a small toast here.
      }
    }
  }

  // ---------- scheduler (min 8s + small jitter to avoid sync) ----------
